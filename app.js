...
$$('#tabs button').forEach(b=>b.addEventListener('click',()=>{
  ...
  const t=b.dataset.tab; $$('main section').forEach(s=>s.classList.remove('active')); document.getElementById(t).classList.add('active');
  if(t==='clientes') renderClientes(); if(t==='movimientos') renderMovimientos(); if(t==='calendario') renderCalendario();
}));

// ---------- Servicios ----------
const DEFAULT_SERVICES = [
  'CANVA','CAPCUT','NETFLIX','YOUTUBE PREMIUM','DISNEY','MAX','CRUNCHYROLL','PARAMOUNT','APPLE TV','VIX','PRIME','CHATGPT','SPOTIFY'
];
async function ensureDefaultServices(){
  const list = await allServicios();
  if(list.length===0){
    for(const n of DEFAULT_SERVICES){ await saveServicio({nombre:n,grupo:'',mensual:0,anual:0}); }
  }
}
async function refreshServiciosSelect(){
  const list=await allServicios();
  const sel=$('#servicio'); if(!sel) return;
  sel.innerHTML='';
  list.sort((a,b)=> (a.nombre||'').localeCompare(b.nombre||''))
      .forEach(s=>{const o=document.createElement('option');o.value=s.nombre;o.textContent=s.nombre;sel.appendChild(o);});
  if(sel.options.length===0){ const o=document.createElement('option'); o.value='SIN ASIGNAR'; o.textContent='SIN ASIGNAR'; sel.appendChild(o); }
}

// ---------- Clientes ----------
async function renderClientes(){
  const data=await allClientes();
  const tb=$('#tablaClientes tbody'); if(!tb) return;
  tb.innerHTML='';
  const hoy=todayStr();
...
function loadClienteToForm(c){
  ...
  $('#email').value = c?.email || '';
  $('#servicio').value = c?.servicio || 'SIN ASIGNAR';
  ...
  $('#fin').value = fmtDate(c?.fin) || todayStr();
}

// Guardado con clave autoincrement (fix: nunca mandamos id=0/''/NaN)
if($('#formCliente')) $('#formCliente').addEventListener('submit', async (e)=>{
  ...
  // Tomar id solo si es un entero > 0
  const rawId = ($('#clienteId').value||'').trim();
  const parsedId = Number.parseInt(rawId, 10);
  const hasValidId = Number.isFinite(parsedId) && parsedId > 0;

  // Defaults
  const servicioSel = $('#servicio').value || 'SIN ASIGNAR';
  const planSel = $('#plan').value || 'mensual';
  const estadoSel = $('#estado').value || 'activo';
  const precioNum = Number($('#precio').value || 0);
  const inicioVal = $('#inicio').value || todayStr();
  const finVal = $('#fin').value || todayStr();

  // Armar objeto SIN id si es nuevo
  const obj = {
  ...
  if (hasValidId) obj.id = parsedId; // solo al editar

  const isNew = !hasValidId;
  try{
    const rid = await saveCliente(obj);
    try { await addMov({tipo:isNew?'ALTA':'EDICIÓN', cliente:(obj.nombre+' '+obj.apellido).trim(), servicio:obj.servicio, plan:obj.plan, monto:obj.precio, notas:isNew?'Alta de cliente':'Edición de datos'});} catch{}
    $('#clienteId').value = (hasValidId ? String(parsedId) : String(rid));
...
if($('#btnRenovar')) $('#btnRenovar').addEventListener('click', async()=>{
  const id=parseInt(($('#clienteId').value||'').trim(),10); if(!Number.isFinite(id)||id<=0) return alert('Seleccioná un cliente');
  ...
  if(np!==null && np.trim()!==''){ const n=Number(np); if(!Number.isNaN(n)) c.precio=n; }
  await saveCliente(c); await addMov({tipo:'RENOVACIÓN', cliente:c.nombre+' '+c.apellido, servicio:c.servicio, plan:c.plan, monto:c.precio, notas:`Nuevo fin: ${c.fin}`});
  await renderClientes(); alert('Renovado');
});
...
if(document.getElementById('exportCSV')) document.getElementById('exportCSV').addEventListener('click', async()=>{
  const data=await allClientes();
...
if(document.getElementById('exportXLS')) document.getElementById('exportXLS').addEventListener('click', async()=>{
  const data=await allClientes();
  const rows=data.map(c=>({
    id:c.id, nombre:c.nombre, apellido:c.apellido, email:c.email, servicio:c.servicio, plan:c.plan, precio:c.precio,
    inicio:fmtDate(c.inicio), fin:fmtDate(c.fin), dias:daysBetween(new Date(), new Date(c.fin)), estado:calcEstado(c.fin).label, notas:c.notas||''
...
if(document.getElementById('exportPDF')) document.getElementById('exportPDF').addEventListener('click', async()=>{
  const data=await allClientes();
  let html='<h1>Clientes</h1><table><thead><tr><th>ID</th><th>Cliente</th><th>Email</th><th>Servicio</th><th>Plan</th><th>Precio</th><th>Inicio</th><th>Fin</th><th>Días</th><th>Estado</th><th>Notas</th></tr></thead><tbody>';
  for(const c of data){
    html+=`<tr><td>${c.id}</td><td>${c.nombre||''} ${c.apellido||''}</td><td>${c.email||''}</td><td>${c.servicio||''}</td><td>${c.plan||''}</td><td>${c.precio||0}</td><td>${fmtDate(c.inicio)}</td><td>${fmtDate(c.fin)}</td><td>${daysBetween(new Date(), new Date(c.fin))}</td><td>${calcEstado(c.fin).label}</td><td>${(c.notas||'').replace(/</g,'&lt;')}</td></tr>`;
  }
  html+='</tbody></table>';
...
function renderCalendario(){
  const el=document.getElementById('gridCal'); if(!el) return;
  el.textContent='(Calendario ok)';
}
...
// ---------- Respaldo ----------
if(document.getElementById('btnBackupJSON')) document.getElementById('btnBackupJSON').addEventListener('click', async()=>{
  const datos={ clientes:await allClientes(), servicios:await allServicios(), movimientos:await allMov() };
  const blob=new Blob([JSON.stringify(datos,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='respaldo_control_cuentas.json'; a.click();
});
if(document.getElementById('btnImportar')) document.getElementById('btnImportar').addEventListener('click', async()=>{
  const f=document.getElementById('fileRestore').files[0]; if(!f) return alert('Selecciona un .json');
  const text=await f.text(); const data=JSON.parse(text);
  if(!confirm('Importar respaldo?')) return;
  ...
  await refreshServiciosSelect(); await renderClientes(); await renderMovimientos();
  alert('Respaldo importado');
});
if(document.getElementById('btnClearDB')) document.getElementById('btnClearDB').addEventListener('click', ()=>{ indexedDB.deleteDatabase('control-cuentas'); alert('Base borrada. Recargá la página.'); });

// ---------- Servicios UI ----------
async function renderServicios(){
  const tb=document.querySelector('#tablaSvc tbody'); if(!tb) return;
  tb.innerHTML='';
  const list=await allServicios();
  list.sort((a,b)=> (a.nombre||'').localeCompare(b.nombre||''));
  for(const s of list){
    const tr=document.createElement('tr');
    tr.innerHTML=`<td>${s.nombre}</td><td>${s.mensual||0}</td><td>${s.anual||0}</td><td>${s.grupo||''}</td>`;
    tr.addEventListener('click',()=>{
      document.getElementById('servicioId').value=s.id||'';
      document.getElementById('svcNombre').value=s.nombre||'';
      document.getElementById('svcGrupo').value=s.grupo||'';
      document.getElementById('svcMensual').value=s.mensual||0;
      document.getElementById('svcAnual').value=s.anual||0;
    });
    tb.appendChild(tr);
  }
}

if(document.getElementById('formServicio')) document.getElementById('formServicio').addEventListener('submit', async (e)=>{
  e.preventDefault();
  const rawId=(document.getElementById('servicioId').value||'').trim();
  const id=parseInt(rawId,10); const hasId=Number.isFinite(id)&&id>0;
  const svc={ nombre:(document.getElementById('svcNombre').value||'').toUpperCase(),
              grupo:document.getElementById('svcGrupo').value||'',
              mensual:Number(document.getElementById('svcMensual').value||0),
              anual:Number(document.getElementById('svcAnual').value||0) };
  if(hasId) svc.id=id;
  await saveServicio(svc);
  await refreshServiciosSelect(); await renderServicios();
...
// ---------- Init ----------
(async function init(){
  try{ await openDB(); }catch(e){ console.error('DB open error', e); alert('Error inicializando base'); return; }
  // Semillas y selects
  await ensureDefaultServices(); await refreshServiciosSelect();
  ...
  await renderClientes(); await renderServicios(); await renderMovimientos();
})();})();